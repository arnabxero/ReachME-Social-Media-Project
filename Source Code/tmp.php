    ...
    function generate_card($uid)
    {
        ...
        while ($class_row = mysqli_fetch_assoc($res)) {
            ...
            if ($class_row['shared'] == 'Y') {
                $main_id = $class_row['shared_pid'];

                $share_sql = "SELECT * FROM posts WHERE id = '$main_id'";
                $share_res = mysqli_query($this->class_con, $share_sql);
                $share_class_row = mysqli_fetch_assoc($share_res);
                $shared = true;

                $share_authorname = $this->get_authorname($share_class_row['authorid']);
                $share_propic_link = $this->get_author_propic($share_class_row['authorid']);
            }
            ...
            if ($shared) {
                ...
                <a title="View User Profile" class="unformatted-link homepage-poster-name" href="view_user.php?uid=' . $share_class_row['authorid'] . '"><img class="profile-pic-home-post" src="' . $share_propic_link . '">&nbsp' . $share_authorname . '<p class="timestamp-home" title="Timestamp & Privacy">' . $share_class_row['time'] . ' &nbsp&nbsp&nbsp ' . $privacy_show . '</p></a>
                ...
                <div class="dropdown" style="float: right; margin-top: -60px;">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                    <li><a class="dropdown-item" href="see_taglist.php?pid=' . $main_id . '"><i class="fas fa-user-tag"></i> See Tagged Users</a></li>
                    </ul>
                </div>
    
    
                <div class="post-text">
                    <span>' . $share_class_row['content'] . '</span>
                </div>';

                if ($share_class_row['media_link'] != "" || !is_null($share_class_row['media_link'])) {
                    echo '
            <div style="margin-bottom: 30px;">
                <' . $media_tag . ' src="' . $share_class_row['media_link'] . '" height="' . $height . '" width="100%" controlsList="nodownload"></' . $media_tag . '>
            </div>';
                }
                ...
                <a href="subdir/post_inter.php?pid=' . $share_class_row['id'] . '&oper=like" title="Upvote">
                <div class="card-button" style="' . $this->like_color . '"><i class="fas fa-thumbs-up"></i> ' . $likes . '</div>
                </a>
                <a href="subdir/post_inter.php?pid=' . $share_class_row['id'] . '&oper=dislike" title="Downvote">
                ...
                <a href="subdir/post_inter.php?pid=' . $share_class_row['id'] . '&oper=share" title="Share">
